...
Do not generate conversation summaries unless explicitly requested.

Do not rewrite project status.

Do not restate completed tasks.

Only report the results of the current task.
___
...
The MASTER_IMPLEMENTATION_PLAN is the single source of truth for implementation scope.

Implement ONLY the requirements explicitly assigned to the current task.

Do NOT infer additional requirements from architecture documents, API documentation, or future milestones.

Documentation may describe the complete product, not the current implementation milestone.

If another document contains functionality that is not explicitly assigned to the current task, ignore it.

Always choose the smallest implementation that completely satisfies the current task.
...
Do not recommend improvements unless explicitly requested.

If a requirement is ambiguous, choose the simplest solution that satisfies the implementation plan.

Never expand task scope on your own.
___
...
- Report only findings that affect the current task.
- Ignore unrelated technical debt.
- Ignore future architectural improvements.
- Ignore possible future optimizations.
- Ignore speculative risks.

Never include sections such as:
...
1. MASTER_IMPLEMENTATION_PLAN
2. Current task description
3. Existing repository architecture
4. Supporting documentation

If documents conflict, follow the higher priority source.

Do not implement functionality that exists only in supporting documentation unless it is explicitly assigned in the implementation plan.

A pre-implementation analysis must answer only:

- What already exists?
- What is missing for the current task?
- What files must change?
- What can be reused?
- Are there dependency conflicts?

Do not design features beyond the current task.

___

# Implementation Workflow

For every implementation task, follow this workflow without exception.

1. Read the task from MASTER_IMPLEMENTATION_PLAN.
2. Produce a Pre-Implementation Validation Report.
3. Validate:
   - scope
   - repository
   - architecture
...
5. Only after approval:
   ...
   - run:
     ...
     - pnpm -r test
     - pnpm -r build
6. Update project summary.
7. Stop.

Never implement before approval.

Never expand task scope beyond MASTER_IMPLEMENTATION_PLAN.

If documentation conflicts with MASTER_IMPLEMENTATION_PLAN, MASTER_IMPLEMENTATION_PLAN takes precedence.
