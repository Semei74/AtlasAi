...
describe("OpenrouterProvider", () => {
  ...
  it("should expose capabilities", () => {
    ...
    expect(provider.capabilities.supportedModels).toContain("openrouter/auto");
  });
  ...
  it("should expose configuration", () => {
    ...
    expect(provider.configuration.baseUrl).toBe("https://openrouter.ai/api/v1");
  });
  ...
  it("should return provider error on chat when no API key", async () => {
    const result = await provider.chat({ model: "openrouter/auto", messages: [{ role: "user", content: "Hi" }], stream: false });
