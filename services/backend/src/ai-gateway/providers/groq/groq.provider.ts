...
export const PROVIDER_GROQ = "PROVIDER_GROQ";
...
export class GroqProvider implements AiProvider {
  public readonly metadata: ProviderMetadata = {
    name: "groq",
    version: "1.0.0",
    description: "Groq provider — real API integration via OpenAI-compatible API",
  };
  ...
  public readonly capabilities: ProviderCapabilities = {
    ...
    maxModels: 0,
    supportedModels: ["llama-4-scout-17b-16e-instruct", "llama-3.3-70b-versatile", "mixtral-8x7b-32768"],
  };
  ...
  public async chat(request: ProviderChatRequest): Promise<ProviderResult<ProviderChatResponse>> {
    ...
    if (key === undefined) {
      return { success: false, latency: 0, error: { code: "NO_API_KEY", message: "Groq API key not configured", statusCode: 502, provider: "groq" } };
    }
    try {
      const maxRetries = this.configuration.maxRetries;
      const baseUrl = this.configuration.baseUrl ?? "https://api.groq.com/openai/v1";
      const body = {
        model: request.model,
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new AiProviderError(`Groq API error ${String(response.status)}: ${errorBody}`);
          }
      ...
      return { success: false, latency: Date.now() - startTime, error: { code: "PROVIDER_ERROR", message: lastError?.message ?? "Groq API error", statusCode: 502, provider: "groq" } };
    } catch (error) {
      return { success: false, latency: Date.now() - startTime, error: { code: "PROVIDER_ERROR", message: error instanceof Error ? error.message : "Unknown Groq error", statusCode: 502, provider: "groq" } };
    }
  }
  ...
  public async health(): Promise<ProviderHealth> {
    ...
    try {
      ...
      return { status: "unhealthy", latency: Date.now() - startTime, lastChecked: new Date(), message: "Cannot reach Groq API" };
    }
  }
