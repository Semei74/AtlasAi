...
export const PROVIDER_XAI = "PROVIDER_XAI";
...
export class XaiProvider implements AiProvider {
  public readonly metadata: ProviderMetadata = {
    name: "xai",
    version: "1.0.0",
    description: "xAI (Grok) provider — real API integration via OpenAI-compatible API",
  };
  ...
  public readonly capabilities: ProviderCapabilities = {
    ...
    maxModels: 0,
    supportedModels: ["grok-3", "grok-3-mini"],
  };
  ...
  public async chat(request: ProviderChatRequest): Promise<ProviderResult<ProviderChatResponse>> {
    ...
    if (key === undefined) {
      return { success: false, latency: 0, error: { code: "NO_API_KEY", message: "xAI API key not configured", statusCode: 502, provider: "xai" } };
    }
    try {
      const maxRetries = this.configuration.maxRetries;
      const baseUrl = this.configuration.baseUrl ?? "https://api.x.ai/v1";
      const body = {
        model: request.model,
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new AiProviderError(`xAI API error ${String(response.status)}: ${errorBody}`);
          }
      ...
      return { success: false, latency: Date.now() - startTime, error: { code: "PROVIDER_ERROR", message: lastError?.message ?? "xAI API error", statusCode: 502, provider: "xai" } };
    } catch (error) {
      return { success: false, latency: Date.now() - startTime, error: { code: "PROVIDER_ERROR", message: error instanceof Error ? error.message : "Unknown xAI error", statusCode: 502, provider: "xai" } };
    }
  }
  ...
  public async health(): Promise<ProviderHealth> {
    ...
    try {
      ...
      return { status: "unhealthy", latency: Date.now() - startTime, lastChecked: new Date(), message: "Cannot reach xAI API" };
    }
  }
