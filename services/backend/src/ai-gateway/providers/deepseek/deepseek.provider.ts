...
export const PROVIDER_DEEPSEEK = "PROVIDER_DEEPSEEK";
...
export class DeepseekProvider implements AiProvider {
  public readonly metadata: ProviderMetadata = {
    name: "deepseek",
    version: "1.0.0",
    description: "DeepSeek provider — real API integration via OpenAI-compatible API",
  };
  ...
  public async chat(request: ProviderChatRequest): Promise<ProviderResult<ProviderChatResponse>> {
    ...
    if (key === undefined) {
      return { success: false, latency: 0, error: { code: "NO_API_KEY", message: "DeepSeek API key not configured", statusCode: 502, provider: "deepseek" } };
    }

    try {
      const maxRetries = this.configuration.maxRetries;
      const baseUrl = this.configuration.baseUrl ?? "https://api.deepseek.com/v1";
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            const msg = `DeepSeek API error ${String(response.status)}: ${errorBody}`;
      ...
      return { success: false, latency: Date.now() - startTime, error: { code: "PROVIDER_ERROR", message: lastError?.message ?? "DeepSeek API error", statusCode: 502, provider: "deepseek" } };
    } catch (error) {
      return { success: false, latency: Date.now() - startTime, error: { code: "PROVIDER_ERROR", message: error instanceof Error ? error.message : "Unknown DeepSeek error", statusCode: 502, provider: "deepseek" } };
    }
  }
  ...
  public async health(): Promise<ProviderHealth> {
    ...
    try {
      ...
      return { status: "unhealthy", latency: Date.now() - startTime, lastChecked: new Date(), message: "Cannot reach DeepSeek API" };
    }
  }
